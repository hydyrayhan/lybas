import { initReactI18next } from "react-i18next";
import i18n from "i18next";

const resources = {
  en: {
    translation: {
      home: "Home",
      dresses: 'Dresses',
      dressmakers: 'Dressmakers',
      blog: 'Blogs',
      search: "Search",
      deliveryAndPaymentOptions: 'Delivery and Payment Options',
      termsOfUse: 'Terms of Use (separate for dressmakers)',
      aboutUs: 'About Us',
      termAndConditions: 'Terms & Conditions',
      contactUs: 'CONTACT US',
      subcribeNewsLetter: 'Subscribe to Our Newsletter',
      enterEmail: "Enter your email",
      allRightsReserved: "2023-2024, All Rights Reserved",
      recommendedDress: "Recommended",
      viewAll: "View all",
      goto: "Go to",
      shop: "shop",
      s: "'s",
      tmt: 'TMT',
      add: "Add",
      mostPopularDresses: "Most Popular",
      more: "More",
      allCategories: "All categories",
      price: 'Price',
      rating: 'Rating',
      inStock: "In Stock",
      province: "Welayat",
      ashgabat: "Ashgabat",
      balkan: 'Balkan',
      ahal: "Ahal",
      mary: "Mary",
      lebap: "Lebap",
      dashoguz: "Dashoguz",
      _2_5days: "Delivery within 2-5 days to welayats",
      fabricName: "Fabric",
      size: 'Size',
      color: "Color",
      comments: "Comments",
      commentOne: "comment",
      commentPageOneComment:"comment",
      commentPageMoreComment:"comments",
      similarDresses: "Similar goods",
      addToCard: "Add to cart",
      numbers: "Quantity",
      total: "Total",
      order: "Order it",
      checkoutNow: "Checkout now",
      seller: "Seller",
      remindMe: "Remind me when in stock",
      remindMeWhenInStock: 'Remind me when in stock',
      remindMeText: 'You will be notified when item will be in stock again.',
      remindMePlaceholder: "Your email*",
      send: 'Send',
      myCart: 'My cart',
      remove: 'Remove',
      totalPrice: "Total",
      or: 'or',
      continueShopping: 'Continue Shopping',
      yourOrder: "Your orders",
      discount: "Discount",
      payment: "Payment",
      cash: "Cash",
      terminal: "Terminal",
      myAccount: "My account",
      profile: "Profile",
      myAddresses: "My address",
      orders: "My orders",
      logout: "Log out",
      edit: "Edit",
      cancel: "Cancel",
      save: "Save",
      addressName: "Address name",
      name: "First name",
      phoneNumber: "Phone number",
      address: "Address",
      addNewAddress: "Add a new address",
      doYouWantToLogout: "Do you really want to log out?",
      ok: "Ok",
      buyAgain: "Buy again",
      user: "User",
      whoWillUseWebsiteAs: "Do you want to be our customer or atelier/dressmaker?",
      customer: "Customer",
      customer2: "Customer",
      dressmaker: "Atelier/Dressmaker",
      hello: "Hello",
      welcomeDashboard: "Welcome back to dashboard",
      users: 'Users',
      comments: "Comments",
      onSale: "On sale",
      sizeChart: "Size chart",
      checkout: "Checkout",
      thereAre: "There are ",
      thereIs: "There is ",
      productsInYourCart: "Products in your cart",
      productInYourCart: "Product in your cart",
      billingDetails: "Delivery details",
      firstName: "First name",
      lastName: "Last name",
      note: "Note",
      yourAddress: "Your address",
      products: "Products",
      quantity: "Quantity",
      subtotal: "Subtotal",
      addAddress: "Add address",
      editPhoto: "Edit photo",
      upload: "Upload",
      delete: "Delete",
      status: "Status",
      orderNumber: "Order number",
      date: "Date",
      leaveComment: "Leave a comment.",
      enterComment: "Enter your comment",
      giveRate: "Thank you for shopping with Lybas. Please rate your purchase and leave a comment.",
      clickToUpload: "Click to Upload",
      addComment: "Add comment",
      myFeedback: "My feedbacks",
      becomeMember: "Become a member",
      signUp: "Sign up",
      signIn: "Sign in",
      password: "Password",
      confirmPassword: "Confirm password",
      rememberMe: "Remember me",
      forgotYourPassword: "Forgot your password?",
      confirm: "Confirm",
      enterTheCode: "Enter the code",
      verificationHelpWord: "Enter the verification code we sent to your phone number",
      bussinessName: "Atelier/brand name",
      verificationCode: "Verification code",
      waitToContact: "Wait to contact",
      waitToContactDefinition: "Thank you! We will contact you within 2-3 business days to verify your account.",
      deliveryAbroad: "Delivery Abroad",
      deliveryAbroadDefinition: "Please enter the URL link of the product you want to be shipped abroad and leave your contact details. We will get in touch with you regarding your inquiry.",
      URLlink: "URL link",
      doYouWantToDelete: "Do you really want to delete?",
      favorites: "Favorites",
      login: "Login",
      deliveryLocation: "Delivery location",
      notification: "Notifications",
      totalBalance: 'Total balance',
      thisMonth: "This month",
      overalRevenue: "Overal Revenue",
      monday: "Mo",
      tuesday: "Tu",
      wednesday: "We",
      thirsday: "Th",
      friday: "Fr",
      saturday: "Sa",
      sunday: "Su",
      next: "Next",
      prev: "Prev",
      reacentOrders: "Reacent Orders",
      addDress: "Add dress",
      searchHere: "Search here...",
      filter: "Filter",
      waiting: "Expected",
      accepted: "Accepted",
      onTheWay: "Delivered",
      cancelled: "Cancelled",
      clientPhoneNumber: "Client, phone number",
      product: "Product",
      number: "Number",
      dateTime: "Date, time",
      statusAccount: "Status",
      result: "Result",
      fullName: "Full name",
      nameOfDressAndDressmaker: "Name of dress and Dressmaker",
      fabric: "Fabric",
      action: "Action",
      aboutTheDress: "About the dress",
      nameTm: "Name tm",
      nameEn: "Name en",
      nameRu: "Name ru",
      category: "Category",
      writeContentTm: "Write content tm",
      writeContentRu: "Write content ru",
      writeContentEn: "Write content en",
      uploadImage: "Upload image",
      nameLocation: "Name, Location",
      commentRating: "Comment, Rating",
      personalInformation: 'Personal Information',
      nameSimple: "Name",
      comment: "Comment",
      link: "Link",
      photos: "Photos",
      photo: "Photo",
      profileImage: "Profile image",
      emailPhoneNumber: "Email, Phone number",
      email: "Email",
      aboutOutStoke: "About out stoke",
      forgotPassword: "Forgot password?",
      heyWelcomeBack: "Hey Welcome Back",
      addDressmaker: "Add dressmaker",
      newDressmakerAdd: "New Dressmaker Add",
      filterName: "Filter name",
      addNewOne: "Add new one",
      image: "Image",
      bannerName: "Banner name",
      banner: "Banner",
      addBanner: 'Add Banner',
      aboutTheBanner: "About the banner",
      startAndEndDate: "Start and End Date",
      blogName: "Blog name",
      text: "Text",
      addBlog: "Add blog",
      addNotification: "Add notification",
      aboutTheNotification: "About the notification",
      textTm: "Text tm",
      textEn: "Text en",
      textRu: "Text ru",
      writeTheDetail: "Write in detail",
      fillTheGaps: "Fill free gaps!",
      type: "Type",
      new: "New",
      new: "New",
      outStock: "Out of stock",
      adminConfirm: "Admin will be give you permission",
      loginWorning: "Login first",
      surname: "Surname",
      typesOfProducts: " types of products",
      addedToCart: "Added to cart",
      noQuantity: "Firstly add quantity",
      changeCart: "Change cart",
      removedFromCart: "Romoved from cart",
      yourAddressIsEmpty: "You don't have address",
      thankYouOrder: "Thank you for your shopping!",
      mailSended: "Email sended",
      thankYouForRate: "We value your feedback",
      goToTheirShop: "Go to their shop",
      successRegister: "You are logged in successfully",
      successSended: "Successfully sended",
      minimum6Letter: "Password must be at least 6 characters long",
      writePhoneNumber: "Write phone number",
      changePassword: "Change password",
      thanksNewsletter: "Thank you for subscribing, we will send updates to your email!",
      orderNotification: "Order completed",
      orderNotificationBody: "You completed order of product please rate it",
      stars: "stars",
      star: "star",
      location: "Location",
      sortBy:"Sort by",
      all:"All",
      acceptedYourSignUp:"We received your sign up request",
      hi:"Hi",
      yourOrderConfirmedNotifStart:"Great news! Your order ",
      yourOrderConfirmedNotifEnd:" has been successfully confirmed. Thank you for choosing Lybas!",
      yourOrderConfirmedNotif2:"Our delivery team will be reaching out shortly to coordinate the delivery. Please ensure your phone is available.",
      yourOrderConfirmedNotif3:"Thank you for shopping with us!",
      welcomeAboardNotif:"Welcome aboard",
      welcomeAboardNotif2:"We're thrilled to have you join Lybas!",
      welcomeAboardNotif3:"Ready to discover the latest trends and must-have Turkmen dresses? Start shopping now ",
      welcomeAboardNotif4:"And don’t forget to fill out your profile and address information on your account page ",
      welcomeAboardNotif5:"Happy shopping!",
      waitingNotif1:"Your order ",
      waitingNotif2:" is in the works, and our operator team will be giving you a call soon to finalize the details.",
      waitingNotif3:"Please keep your phone close by for their call. We are here to ensure your purchase will be smooth!",
      waitingNotif4:"Thank you for choosing Lybas!",
      rateNotif1:"We hope you're enjoying your recent purchase from Lybas! Your opinion matters to us and fellow shoppers. Could you take a moment to leave a review and share your experience?",
      rateNotif2:"Thank you for being a valued part of our community!",
      goOrder:"Go order page",
      information:"Informations",
      orderLaw: `<ul class='list-disc ml-5'>
                  <li>Eltip bermek hyzmaty Aşgabat şäheriniň çägi bilen bir hatarda Büzmeýine we Änew şäherine hem elýeterlidir;</li>
                  <li>Sargydyň iň pes çägi 50 manat bolmaly; 50-150 manat arasy eltip bermek hyzmatynyň tölegi 10 manat; sargydyňyz 150 manatdan geçse eltip bermek hyzmaty mugt;</li>
                  <li>Saýtdan sargyt edeniňizden soňra operator size jaň edip sargydy tassyklar (eger hemişelik müşderi bolsaňyz sargytlaryňyz islegiňize görä awtomatik usulda hem tassyklanýar);</li>
                  <li>Sargydy barlap alanyňyzdan soňra töleg amala aşyrylýar. Eltip berijiniň size gowşurýan töleg resminamasynda siziň tölemeli puluňyz bellenendir. Töleg nagt görnüşde milli manatda amala aşyrylýar. Kabul edip tölegini geçiren harydyňyz yzyna alynmaýar.</li>
                </ul>`,
      aboutUsMain: "Welcome to Lybas, where the rich tapestry of Turkmen craftsmanship comes to life in the form of exquisite dresses and accessories, handcrafted by talented local ateliers and dressmakers. We invite you to embark on a journey through our virtual marketplace, where tradition meets modernity, and every purchase tells a unique story.",
      aboutUsTitle1: "Our Vision: ",
      aboutUsBody1: "At Lybas, we envision a world where the artistry of Turkmen women entrepreneurs is celebrated and shared with the global community. Our platform is a testament to the resilience, creativity, and skill of local ateliers and dressmakers who pour their hearts into creating one-of-a-kind pieces that reflect the vibrant cultural heritage of Turkmenistan.",
      aboutUsTitle2: 'Empowering Women Entrepreneurs: ',
      aboutUsBody2: "Our marketplace is more than just an online shopping destination; it's a celebration of the entrepreneurial spirit of Turkmen women. By curating a diverse collection of custom-made dresses and accessories, we provide a platform for local businesses to showcase their talents and reach a wider audience. Your purchase not only adds a unique piece to your wardrobe but also contributes to the growth and empowerment of these inspiring women.",
      aboutUsTItle3: "Unique Custom-Made Creations: ",
      aboutUsBody3: "Discover the allure of Turkmen fashion through our curated selection of custom-made dresses and accessories. Each piece is a labor of love, blending traditional techniques with contemporary styles to create something truly extraordinary. From vibrant dresses adorned with intricate embroidery to accessories that accentuate your individuality, our marketplace is a treasure trove of handcrafted wonders.",
      aboutUsTitle4: "Seamless Online Shopping Experience: ",
      aboutUsBody4: "We understand the importance of convenience in your shopping journey. Navigating our platform is a breeze, allowing you to explore, select, and purchase with ease. Our user-friendly interface ensures that you can browse through a wide array of options, read about the talented artisans behind each creation, and enjoy shopping from the comfort of your home.",
      aboutUsBody5: "Thank you for being a part of Lybas. As you explore our collection, we hope you find joy in each unique creation and take pride in supporting the flourishing community of Turkmen women entrepreneurs. Welcome to a world where tradition, empowerment, and style converge—welcome to Lybas!",
      deliveryAndPaymentHead: "Delivery and Payment Options",
      deliveryAndPaymentBody: `        
                              <li>We offer delivery to all velayats of our country. The delivery time may vary depending on the location of your product. For products in Ashgabat, deliveries are made within 24 hours.</li>
                              <div class="ml-5">Ashgabat: 20 manats</div>
                              <div class="ml-5">Anew: 30-40 manats</div>
                              <div class="ml-5">Buzmeyin: 50 manats</div>
                              <div class="ml-5">Choganly: 30-40 manats</div>
                              <div class="ml-5">Gokdepe: 50 manats</div>
                              <div class="ml-5">Gurtly: 25 manats</div>
                              <div class="ml-5">Yalkym: 40 manats</div>
                              <div class="ml-5">Tejen: 50 manats</div>
                              <li>For all other velayats, deliveries may take up to 2-5 days to receive your shipment. The delivery price may range between 50-90 manats to velayats depending on your location.</li>
                              <li>After you complete your order on our website, our operator will call you to confirm the order.</li>
                              <li>The delivery service is carried out to the address and telephone number you specified. </li>
                              <li>If the order is not confirmed, then the order is not registered and is not executed. To confirm your order, make sure you can receive a call at the phone number you provided. Also, please check that the address you provided is correct.</li>
                              <li>Payment is made after you check and accept the order. The courier's payment document indicates the amount of your payment. Delivery amount will be added to your total amount of the product due. Payment is made in cash in national currency. Accepted and paid for goods cannot be returned. </li>
                              <li>If the owner of the order cannot be found within 24 hours after confirmation of the order, then the order is cancelled.</li>`,
    },
  },

  // //////////////////////////////////////////////////////////////////////////////////////////////////////////////////


  ru: {
    translation: {
      home: "Главная",
      dresses: 'Платья',
      dressmakers: 'Портнихи',
      blog: 'Блоги',
      search: "Поиск",
      deliveryAndPaymentOptions: 'Порядок доставки и оплаты',
      termsOfUse: 'Условия использования и политика конфиденциальности',
      aboutUs: 'О нас',
      termAndConditions: 'Условия использования и политика конфиденциальности',
      contactUs: 'СВЯЗАТЬСЯ С НАМИ',
      subcribeNewsLetter: 'Подписывайтесь на нашу рассылку',
      enterEmail: "Введите адрес электронной почты",
      allRightsReserved: "2023-2024, Все права защищены.",
      recommendedDress: "Рекомендуемые",
      viewAll: "Посмотреть все",
      goto: "Перейти в магазин",
      shop: "shop ru",
      s: "'s",
      tmt: 'TMT',
      add: "Добавить",
      mostPopularDresses: "Самые популярные",
      more: "Подробнее",
      allCategories: "Все категории",
      price: 'Цена',
      rating: 'Рейтинг',
      inStock: "В наличии",
      province: "Велаят",
      ashgabat: "Ашхабад",
      balkan: 'Балкан ',
      ahal: "Ахал",
      mary: "Мары",
      lebap: "Лебап",
      dashoguz: "Дашогуз",
      _2_5days: "Доставка в течение 2-5 дней в велаяты.",
      fabricName: "Ткань",
      size: 'Размер',
      color: "Цвета",
      comments: "Отзывы",
      commentOne: "комментариев",
      commentPageOneComment:"отзыв",
      commentPageMoreComment:"отзыва",
      similarDresses: "Похожие товары",
      addToCard: "Добавить в корзину",
      numbers: "Количество",
      total: "Общая сумма",
      order: "Заказать",
      checkoutNow: "Оформить заказ",
      seller: "Продавец",
      remindMe: "Напомните, когда снова в наличии",
      remindMeWhenInStock: 'Напомните, когда снова в наличии',
      remindMeText: 'Вы будете уведомлены, когда товар снова появится в наличии.',
      remindMePlaceholder: "Ваш адрес электронной почты*",
      send: 'Отправить',
      myCart: 'Моя корзина',
      remove: 'Удалить',
      totalPrice: "Общая сумма",
      or: 'или',
      continueShopping: 'продолжить покупку',
      yourOrder: "Ваши заказы",
      discount: "Discount",
      payment: "Payment",
      cash: "Cash",
      terminal: "Terminal",
      myAccount: "Мой аккаунт",
      profile: "Профиль",
      myAddresses: "Мой адрес",
      orders: "Мои заказы",
      logout: "Выйти",
      edit: "Редактировать",
      cancel: "Отмена",
      save: "Сохранить",
      addressName: "Мой адрес",
      name: "Имя",
      phoneNumber: "Номер телефона",
      address: "Ваш адрес",
      addNewAddress: "Добавить новый адрес",
      doYouWantToLogout: "Вы действительно хотите выйти из системы?",
      ok: "Ок",
      buyAgain: "Buy again",
      user: "Пользователь",
      whoWillUseWebsiteAs: "Хотите стать нашим клиентом или ателье/портнихой?",
      customer: "Клиентом",
      customer2: "Клиент",
      dressmaker: "Ателье/Портниха",
      hello: "Привет",
      welcomeDashboard: "Добро пожаловать обратно на панель управления",
      users: 'Users',
      comments: "Отзывы",
      onSale: "Скидки",
      sizeChart: "Таблица размеров",
      checkout: "Оформить заказ",
      thereAre: "В вашей корзине ",
      thereIs: "В вашей корзине ",
      productsInYourCart: " товара",
      productInYourCart: " товар",
      billingDetails: "Детали доставки",
      firstName: "Имя",
      lastName: "Фамилия",
      note: "Примечания",
      yourAddress: "Ваш адрес",
      products: "Товари",
      quantity: "Количество",
      subtotal: "Итого",
      addAddress: "Добавить адрес",
      editPhoto: "Редактировать фотографию",
      upload: "Загрузить",
      delete: "Удалить",
      status: "Статус заказа",
      orderNumber: "Номер заказа",
      date: "Дата",
      leaveComment: "Oставьте отзыв.",
      enterComment: "Введите свой отзыв",
      giveRate: "Благодарим вас за покупку с Lybas. Пожалуйста, оцените покупку и оставьте отзыв.",
      clickToUpload: "Нажмите, чтобы загрузить",
      addComment: "Добавить отзыв",
      myFeedback: "Мои отзывы",
      becomeMember: "Становиться участником",
      signUp: "Зарегистрироваться",
      signIn: "Войти",
      password: "Пароль",
      confirmPassword: "Подтвердите пароль",
      rememberMe: "Запомнить меня",
      forgotYourPassword: "Забыли пароль?",
      confirm: "Подтверждать",
      enterTheCode: "Введите код",
      verificationHelpWord: "Введите код подтверждения, который мы отправили на ваш номер телефона",
      bussinessName: "Название ателье/брэнда",
      verificationCode: "Код подтверждения",
      waitToContact: "Подождите, чтобы связаться",
      waitToContactDefinition: "Спасибо! Мы свяжемся с вами в течение 2-3 рабочих дней для проверки вашей учетной записи.",
      deliveryAbroad: "Доставка за границу",
      deliveryAbroadDefinition: "Пожалуйста, введите URL-ссылку на товар, который вы хотите отправить за границу, и оставьте свои контактные данные. Мы свяжемся с вами по поводу вашего запроса.",
      URLlink: "URL-ссылка",
      doYouWantToDelete: "Вы действительно хотите удалить?",
      favorites: "Избранное",
      login: "Войти",
      deliveryLocation: "Адрес доставки",
      notification: "Уведомления",
      totalBalance: 'Общая сумма',
      thisMonth: "Этот месяц",
      overalRevenue: "Общий доход",
      monday: "По",
      tuesday: "Вт",
      wednesday: "Ср",
      thirsday: "Че",
      friday: "Пя",
      saturday: "Су",
      sunday: "Во",
      next: "След",
      prev: "Пред",
      reacentOrders: "Недавние заказы",
      addDress: "Добавить платье",
      searchHere: "Поищи здесь...",
      filter: "Филтер",
      waiting: "Ожидается",
      accepted: "Принято",
      onTheWay: "Доставлено",
      cancelled: "Отменено",
      clientPhoneNumber: "Клиент, номер телефона",
      product: "Товар",
      number: "Число",
      dateTime: "Дата, время",
      statusAccount: "Статус заказа",
      result: "Результат",
      fullName: "Полное имя",
      nameOfDressAndDressmaker: "Название платья и портниха",
      fabric: "Ткань",
      action: "Действие",
      aboutTheDress: "О платье",
      nameTm: "Имя tm",
      nameEn: "Имя en",
      nameRu: "Имя ru",
      category: "Категория",
      writeContentTm: "Написать контент tm",
      writeContentRu: "Написать контент ru",
      writeContentEn: "Написать контент en",
      uploadImage: "Загрузить изображение",
      nameLocation: "Имя, Местоположение",
      commentRating: "Комментарий, Рейтинг",
      personalInformation: 'Персональная информация',
      nameSimple: "Имя",
      comment: "Комментарий",
      link: "Cсылки",
      photos: "Фото",
      photo: "Фото",
      profileImage: "Изображение профиля",
      emailPhoneNumber: "Электронная почта, номер телефона",
      email: "Электронная почта",
      aboutOutStoke: "О нашем стоке",
      forgotPassword: "Забыли пароль?",
      heyWelcomeBack: "Привет, с возвращением",
      addDressmaker: "Добавить портниху",
      newDressmakerAdd: "Новая портниха Добавить",
      filterName: "Название фильтра",
      addNewOne: "Добавить новый",
      image: "Изображение",
      bannerName: "Название баннера",
      banner: "Баннер",
      addBanner: 'Добавить баннер',
      aboutTheBanner: "О баннере",
      startAndEndDate: "Дата начала и окончания",
      blogName: "Название блога",
      text: "Текст",
      addBlog: "Добавить блог",
      addNotification: "Добавить уведомление",
      aboutTheNotification: "Об уведомлении",
      textTm: "Текст tm",
      textEn: "Текст en",
      textRu: "Текст ru",
      writeTheDetail: "Напишите подробно",
      fillTheGaps: "Заполните свободные пробелы!",
      type: "Тип",
      new: "Новый",
      outStock: "Нет в наличии",
      adminConfirm: "Администратор даст вам разрешение",
      loginWorning: "Сначала войдите в систему",
      surname: "Фамилия",
      typesOfProducts: " видов товаров",
      addedToCart: "Добавлено в корзину",
      noQuantity: "Сначала добавьте количество",
      changeCart: "Изменить корзину",
      removedFromCart: "Удалено из корзины",
      yourAddressIsEmpty: "У вас нет адреса",
      thankYouOrder: "Спасибо за покупки!",
      mailSended: "Электронная почта отправлено",
      thankYouForRate: "Мы ценим ваши отзывы",
      goToTheirShop: "Перейти в магазин",
      successRegister: "Вы успешно вошли в систему",
      successSended: "Успешно отправлено",
      minimum6Letter: "Пароль должен содержать как минимум 6 символов",
      writePhoneNumber: "Напишите номер телефона",
      changePassword: "Изменить пароль",
      thanksNewsletter: "Спасибо за подписку, мы будем присылать обновления на вашу электронную почту!",
      orderNotification: "Заказ выполнен",
      orderNotificationBody: "Вы завершили заказ товара, пожалуйста, оцените его",
      stars: "звезды",
      star: "звезда",
      location: "Местонахождение",
      sortBy:"Сортировать по",
      all:"Все",
      acceptedYourSignUp:"Мы получили ваш запрос на регистрацию",
      hi:"Здравствуйте",
      yourOrderConfirmedNotifStart:"Отличная новость! Ваш заказ ",
      yourOrderConfirmedNotifEnd:" успешно подтвержден. Спасибо, что выбрали Lybas!",
      yourOrderConfirmedNotif2:"В ближайшее время наша команда доставки свяжется с вами для координации доставки. Убедитесь, что ваш телефон доступен.",
      yourOrderConfirmedNotif3:"Спасибо за покупку!",
      welcomeAboardNotif:"Добро пожаловать",
      welcomeAboardNotif2:"Мы очень рады, что вы присоединились к Lybas!",
      welcomeAboardNotif3:"Готовы узнать о последних тенденциях и актуальных туркменских платьях? Начните делать покупки прямо сейчас ",
      welcomeAboardNotif4:"И не забудьте заполнить свой профиль и адресную информацию на странице своего аккаунта ",
      welcomeAboardNotif5:"Удачных покупок!",
      waitingNotif1:"Ваш заказ ",
      waitingNotif2:" находится в обработке, и в ближайшее время вам позвонят наши операторы, чтобы уточнить детали.",
      waitingNotif3:"Пожалуйста, держите телефон под рукой на случай звонка. Мы здесь для того, чтобы ваша покупка прошла гладко!",
      waitingNotif4:"Благодарим вас за выбор Lybas!",
      rateNotif1:"Мы надеемся, что вам понравилась недавняя покупка в Lybas! Ваше мнение важно для нас и других покупателей. Не могли бы вы найти минутку, чтобы оставить отзыв и поделиться своим опытом?",
      rateNotif2:"Благодарим вас за то, что вы являетесь ценной частью нашего сообщества!",
      goOrder:"Перейти на страницу заказа",
      information:"Информация",
      orderLaw: `<ul class='list-disc ml-5'>
                  <li>Eltip bermek hyzmaty Aşgabat şäheriniň çägi bilen bir hatarda Büzmeýine we Änew şäherine hem elýeterlidir;</li>
                  <li>Sargydyň iň pes çägi 50 manat bolmaly; 50-150 manat arasy eltip bermek hyzmatynyň tölegi 10 manat; sargydyňyz 150 manatdan geçse eltip bermek hyzmaty mugt;</li>
                  <li>Saýtdan sargyt edeniňizden soňra operator size jaň edip sargydy tassyklar (eger hemişelik müşderi bolsaňyz sargytlaryňyz islegiňize görä awtomatik usulda hem tassyklanýar);</li>
                  <li>Sargydy barlap alanyňyzdan soňra töleg amala aşyrylýar. Eltip berijiniň size gowşurýan töleg resminamasynda siziň tölemeli puluňyz bellenendir. Töleg nagt görnüşde milli manatda amala aşyrylýar. Kabul edip tölegini geçiren harydyňyz yzyna alynmaýar.</li>
                </ul>`,
      aboutUsMain: 'Добро пожаловать на веб-сайт Лыбас, где богатое туркменское мастерство воплощается в жизнь в виде изысканных платьев и аксессуаров, созданных вручную талантливыми местными ателье и портнихами. Мы приглашаем вас отправиться в путешествие по нашему виртуальному рынку, где традиции встречаются с современностью, а каждая покупка рассказывает уникальную историю.',
      aboutUsTitle1: 'Наше видение:',
      aboutUsBody1: 'В Лыбас мы создаем мир, в котором искусство туркменских женщин-предпринимателей доступно мировому сообществу. Наша платформа является свидетельством стойкости, креативности и мастерства местных ателье и портных, которые вкладывают всю свою душу в создание уникальных изделий, отражающих яркое культурное наследие Туркменистана.',
      aboutUsTitle2: 'Расширение бизнеса женщин-предпринимателей:',
      aboutUsBody2: 'Наша торговая площадка — это больше, чем просто место для покупок в Интернете; это празднование предпринимательского духа туркменских женщин. Собирая разнообразную коллекцию платьев и аксессуаров ручной работы, мы предоставляем местным ателье и портнихам платформу, позволяющую продемонстрировать свои таланты и охватить более широкую аудиторию. Ваша покупка не только добавит уникальный элемент в ваш гардероб, но и будет способствовать росту и расширению бизнеса этих вдохновляющих женщин.',
      aboutUsTItle3: 'Уникальные изделия:',
      aboutUsBody3: 'Откройте для себя очарование туркменской моды с помощью тщательно подобранной коллекции платьев и аксессуаров. Каждое изделие — это плод любви, сочетание традиционных техник с современными стилями для создания чего-то поистине экстраординарного. От ярких платьев, украшенных замысловатой вышивкой, до аксессуаров, подчеркивающих вашу индивидуальность, Лыбас — настоящая сокровищница чудес ручной работы.',
      aboutUsTitle4: 'Приятный опыт покупок в Интернете:',
      aboutUsBody4: 'Мы понимаем важность удобства в вашем путешествии по магазинам. Навигация по нашей платформе очень проста, что позволяет вам с легкостью искать, выбирать и приобретать товары. Наш удобный интерфейс гарантирует, что вы сможете просматривать широкий спектр вариантов, читать о талантливых мастерах, стоящих за каждым творением, и совершать покупки, не выходя из дома.',
      aboutUsBody5: 'Благодарим вас за то, что вы являетесь неотъемлемой частью Лыбас. Мы надеемся, что, ознакомясь с нашей коллекцией, вы получите радость от каждого уникального творения и гордиться тем, что поддерживаете процветающее сообщество туркменских женщин предпринимателей. Добро пожаловать в мир, где сливаются традиции, возможности и стиль — добро пожаловать на Лыбас!',
      deliveryAndPaymentHead: "Порядок доставки и оплаты",
      deliveryAndPaymentBody: `        
                              <li>Мы предлагаем доставку во все велаяты нашей страны. Срок доставки может варьироваться в зависимости от местонахождения вашего товара. Доставка продукции по Ашхабаду осуществляется в течение 24 часов в пределах города.</li>
                              <div class="ml-5">Ашхабад: 20 манатов</div>
                              <div class="ml-5">Аннау: 30-40 манатов.</div>
                              <div class="ml-5">Бузмеин: 50 манатов</div>
                              <div class="ml-5">Чоганлы: 30-40 манатов</div>
                              <div class="ml-5">Гёкдепе: 50 манатов</div>
                              <div class="ml-5">Гуртлы: 25 манат</div>
                              <div class="ml-5">Ялкым: 40 манатов</div>
                              <div class="ml-5">Теджен: 50 манатов</div>
                              <li>Во все остальные велаяты доставка может занять до 2-5 дней. Цена доставки в велаяты может составлять 50-90 манатов в зависимости от вашего местоположения. </li>
                              <li>После того, как вы оформите заказ на нашем сайте, вам позвонит наш оператор для подтверждения заказа.</li>
                              <li>Доставка осуществляется по указанному вами адресу и телефону.</li>
                              <li>Если заказ не подтвержден, то заказ не регистрируется и не исполняется. Для подтверждения заказа обязательно ответьте на звонок по указанному вами номеру телефона. Также проверьте правильность указанного вами адреса.</li>
                              <li>Оплата производится после проверки и принятия заказа. В платежном документе курьера указана сумма вашего платежа. Сумма доставки будет добавлена к общей сумме товара, подлежащего оплате. Оплата производится наличными в национальной валюте. Принятый и оплаченный товар возврату не подлежит.</li>
                              <li>Если владелец заказа не может быть найден в течение 24 часов после подтверждения заказа, то заказ аннулируется.</li>`
    },
  },




  // /////////////////////////////////////////////////////////////////////////////////////////////////////




  tm: {
    translation: {
      home: "Baş sahypa",
      dresses: 'Köýnekler',
      dressmakers: 'Tikinçiler',
      blog: 'Bloglar',
      search: "Gözlemek",
      deliveryAndPaymentOptions: 'Eltip bermek we töleg tertibi',
      termsOfUse: 'Ulanyş düzgünleri we gizlinlik şertleri ',
      aboutUs: 'Biz barada',
      termAndConditions: 'Ulanyş düzgünleri we gizlinlik şertleri',
      contactUs: 'Habarlaşmak üçin',
      subcribeNewsLetter: 'Täzeliklere ýazylyň',
      enterEmail: "E-poçtaňyzy giriziň",
      allRightsReserved: "2023-2024, Ähli hukuklar goraglydyr",
      recommendedDress: "Hödürlenýänler",
      viewAll: "Ählisi",
      goto: "Görmek",
      shop: "dükanyny",
      s: "-yň",
      tmt: "TMT",
      add: "Goşmak",
      mostPopularDresses: "Saýlananlar",
      more: "Giňişleýin",
      allCategories: "Ähli kategoriýalar",
      price: 'Bahasy',
      rating: 'Reýting',
      inStock: "Elimizde bar",
      province: "Welaýat",
      ashgabat: "Aşgabat",
      balkan: 'Balkan',
      ahal: "Ahal",
      mary: "Mary",
      lebap: "Lebap",
      dashoguz: "Daşoguz",
      _2_5days: "Welaýatlara eltip bermek wagty 2-5 gün.",
      fabricName: "Matasy",
      size: 'Ölçegi',
      color: "Reňkler",
      comments: "Synlar",
      commentOne: "syn",
      commentPageOneComment:"syn",
      commentPageMoreComment:"syn",
      similarDresses: "Meňzeş harytlar",
      addToCard: "Sebete goşmak",
      numbers: "Mukdary",
      total: "Jemi",
      order: "Sargamak",
      checkoutNow: "Sargamak",
      seller: "Satyjy",
      remindMe: "Gelende ýatladyň",
      remindMeWhenInStock: 'Gelende ýatladyň',
      remindMeText: 'Haryt elimize gelende size habar bereris.',
      remindMePlaceholder: "E-poçta salgyňyz*",
      send: 'Ibermek',
      myCart: 'Sebedim',
      remove: 'Pozmak',
      totalPrice: "Umumy jemi",
      or: 'ýa-da',
      continueShopping: 'söwda etmäge dowam etmek',
      yourOrder: "Sargytlaryňyz",
      discount: "Discount",
      payment: "Payment",
      cash: "Cash",
      terminal: "Terminal",
      myAccount: "Meniň akkaundym",
      profile: "Profil",
      myAddresses: "Öý salgym",
      orders: "Sargytlarym",
      logout: "Çykmak",
      edit: "Redaktirlemek",
      cancel: "Goýbolsun etmek",
      save: "Ýatda saklamak",
      addressName: "Öý salgym",
      name: "Adyňyz",
      phoneNumber: "Telefon belgiňiz",
      address: "Salgyňyz",
      addNewAddress: "Täze salgy goşmak",
      doYouWantToLogout: "Hakykatdan çykmak isleýärsiňizmi?",
      ok: "Ok",
      buyAgain: "Buy again", //
      user: "Ulanyjy", //
      whoWillUseWebsiteAs: "Müşderimiz hökmündemi ýa-da atelýe /tikinçi hökmünde agza bolmak isleýärsiňiz? Birini saýlaň?",
      customer: "Müşderi",
      customer2: "Müşderi",
      dressmaker: "Atelýe / Tikinçi",
      hello: "Salam",
      welcomeDashboard: "Hoş geldiňiz",
      users: 'Users',
      comments: "Synlar",
      onSale: "Arzanladyşdakylar",
      sizeChart: "Ölçeg tablisasy",
      checkout: "Sargamak",
      thereAre: "Sebetiňizde ",
      thereIs: "Sebetiňizde ",
      productsInYourCart: " haryt bar",
      productInYourCart: " haryt bar",
      billingDetails: "Eltip bermek üçin maglumatlar",
      firstName: "Adyňyz",
      lastName: "Familiýaňyz",
      note: "Bellik",
      yourAddress: "Öý salgyňyz",
      products: "Harytlar",
      quantity: "Mukdary",
      subtotal: "Jemi",
      addAddress: "Salgy goşmak",
      editPhoto: "Suraty redaktirlemek",
      upload: "Ýüklemek",
      delete: "Pozmak",
      status: "Sargyt ýagdaýy",
      orderNumber: "Sargyt belgisi",
      date: "Senesi",
      leaveComment: "Syn ýazmagyňyzy haýyş edýäris.",
      enterComment: "Synyňyzy ýazyň",
      giveRate: "Lybas bilen söwdaňyz üçin sag boluň. Satyn alan harydyňyza ýyldyz bilen baha berip syn ýazmagyňyzy haýyş edýäris.",
      clickToUpload: "Ýüklemek üçin basyň",
      addComment: "Syn goşmak",
      myFeedback: "Synlarym",
      becomeMember: "Become a member", //Gerekdal
      signUp: "Agza bolmak",
      signIn: "Girmek",
      password: "Parol",
      confirmPassword: "Paroly tassyklamak",
      rememberMe: "Meni ýatda saklamak",
      forgotYourPassword: "Parolyňyzy ýatdan çykardyňyzmy?",
      confirm: "Tassyklamak",
      enterTheCode: "Kody girizmek",
      verificationHelpWord: "Telefon belgiňize iberen tassyklama kody giriziň",
      bussinessName: "Atelýeňiziň / brendiňiziň ady",
      verificationCode: "Tassyklama kody",
      waitToContact: "Habarlaşýançak garaşyň",
      waitToContactDefinition: "Sagboluň! Akkaundyňyzy tassyklamak üçin 2-3 iş gününiň dowamynda siz bilen habarlaşarys.",
      deliveryAbroad: "Daşary ýurtda eltip bermek",
      deliveryAbroadDefinition: "Daşary ýurda ibermek isleýän harydyňyzyň URL ssylkasyny giriziň we aragatnaşyk maglumatlaryňyzy ýazyň. Siz bilen habarlaşarys.",
      URLlink: "URL ssylka",
      doYouWantToDelete: "Siz hakykatdanam pozmak isleýäňizmi?",
      favorites: "Halanlarym",
      login: "Girmek",
      deliveryLocation: "Eltip bermek salgysy",
      notification: "Täzelikler",
      totalBalance: 'Jemi',
      thisMonth: "Şu aý",
      overalRevenue: "Overal Revenue", //
      monday: "Mo",
      tuesday: "Tu",
      wednesday: "We",
      thirsday: "Th",
      friday: "Fr",
      saturday: "Sa",
      sunday: "Su",
      next: "Indiki",
      prev: "Öňki",
      reacentOrders: "Ýakyndaky zakazlar",
      addDress: "Köýnek goşmak",
      searchHere: "Gözlemek ...",
      filter: "Filter",
      waiting: "Garaşylýar",
      accepted: "Kabul edildi",
      onTheWay: "Gowşuryldy",
      cancelled: "Goýbolsun edildi",
      clientPhoneNumber: "Müşderi, telefon belgi",
      product: "Haryt",
      number: "Nomer",
      dateTime: "Sene, wagt",
      statusAccount: "Sargyt ýagdaýy",
      result: "Netije",
      fullName: "Doly ady",
      nameOfDressAndDressmaker: "Köýnegiň we Dikimçiň/Otelýaň ady",
      fabric: "Matasy",
      action: "Action",
      aboutTheDress: "Köýnek barada",
      nameTm: "Ady tm",
      nameEn: "Ady en",
      nameRu: "Ady ru",
      category: "Kategoriýa",
      writeContentTm: "Tekst ýazyň tm",
      writeContentRu: "Tekst ýazyň ru",
      writeContentEn: "Tekst ýazyň en",
      uploadImage: "Surat ýüklemek",
      nameLocation: "Ady, Ýerleşýän ýeri",
      commentRating: "Syn, rating",
      personalInformation: 'Şahsy Maglumat',
      nameSimple: "Ady",
      comment: "Syn",
      link: "Link",
      photos: "Suratlar",
      photo: "Surat",
      profileImage: "Profil surat",
      emailPhoneNumber: "Email, Telefon belgi",
      email: "Email",
      aboutOutStoke: "Elimizde ýoklar barada",
      forgotPassword: "Açar sözümi unutdum?",
      heyWelcomeBack: "Hoş geldiň",
      addDressmaker: "Dikimçi/Otelýa goşmak",
      newDressmakerAdd: "Täze Dikimçi/Otelýa goşmak",
      filterName: "Filter ady",
      addNewOne: "Täze birini goşmak",
      image: "Surat",
      bannerName: "Banner Ady",
      banner: "Banner",
      addBanner: 'Banner goşmak',
      aboutTheBanner: "Banner barada",
      startAndEndDate: "Başlaýan we gutarýan senesi",
      blogName: "Blog Ady",
      text: "Tekst",
      addBlog: "Blog goşmak",
      addNotification: "Täzelik goşmak",
      aboutTheNotification: "Täzelik barada",
      textTm: "Tekst tm",
      textEn: "Tekst en",
      textRu: "Tekst ru",
      writeTheDetail: "Maglumat ýaz",
      fillTheGaps: "Boşluklary dolduryň!",
      type: "Görnüşi",
      new: "Täze",
      outStock: "Elimizde ýok",
      adminConfirm: "Admin rugsat bermeli",
      loginWorning: "Ilki registrasiýa boluň",
      surname: "Familiýa",
      typesOfProducts: " sany haryt görnüşi",
      addedToCart: "Sebede goşuldy",
      noQuantity: "Haryt mukdaryny goşuň",
      changeCart: "Sebedi üýtget",
      removedFromCart: "Sebetden pozuldy",
      yourAddressIsEmpty: "Siziň salgyňyz ýok",
      thankYouOrder: "Söwdaňyz üçin sagboluň!",
      mailSended: "Email ugradyldy",
      thankYouForRate: "Siziň synyňyz biziň üçin wajypdyr.",
      goToTheirShop: "Dükanyna geçmek",
      successRegister: "Siz üstünlikli girdiňiz",
      successSended: "Üstünlikli ugradyldy",
      minimum6Letter: "Parol azyndan 6 simwoldan ybarat bolmaly",
      writePhoneNumber: "Telefon belgiňizi giriziň",
      changePassword: "Paroly üýtget",
      thanksNewsletter: "Sagboluň, ýazylanyňyz üçin, Siziň emailiňize täzelikleri ibereris!",
      orderNotification: "Sargyt tamamlandy",
      orderNotificationBody: "Sargyt tamamlandy, bermegiňizi haýyş edýäris",
      stars: "ýyldyz",
      star: "ýyldyz",
      location: "Ýeri",
      sortBy:"Tertibi",
      all:"Hemmesi",
      acceptedYourSignUp:"Siziň agza bolmak haýyşyňyzy aldyk",
      hi:"Salam",
      yourOrderConfirmedNotifStart:"Gowy habarymyz bar! Belgili ",
      yourOrderConfirmedNotifEnd:" sargydyňyz üstünlikli tassyklandy. Lybasy saýlanyňyz üçin sag boluň!",
      yourOrderConfirmedNotif2:"В Eltip beriş toparymyz gysga wagtda siziň bilen habarlaşar. Telefonyňyzyň elýeterlidigine göz ýetirmegiňizi haýyş edýäris.",
      yourOrderConfirmedNotif3:"Söwdaňyz üçin sag boluň!",
      welcomeAboardNotif:"Hoş geldiňiz",
      welcomeAboardNotif2:"Lybasa goşulýandygyňyza biz örän şat!",
      welcomeAboardNotif3:"Iň täze moda we türkmen köýnekleri barada bilesiňiz gelýärmi? Şuwagt söwda edip başlaň ",
      welcomeAboardNotif4:"Öz akkaunt sahypaňyzdaky profiliňizi we salgy maglumatlaryňyzy doldurmagy ýatdan çykarmaň ",
      welcomeAboardNotif5:"Söwdaňyz şowly bolsun!",
      waitingNotif1:"",
      waitingNotif2:" belgili sargydyňyz gözden geçirilýär we beren maglumatlaryňyzy anyklamak üçin operatorlarymyz gysga wagtda size jaň ederler.",
      waitingNotif3:"Telefonyňyzyň elýeterlidigine göz ýetirmegiňizi haýyş edýäris. Söwdaňyzyň oňaýly bolmagy üçin zähmet çekýäris.",
      waitingNotif4:"Lybasy saýlanyňyz üçin sag boluň!",
      rateNotif1:"Lybasdan soňky satyn alan harydyňyz göwnüňizden turandyr diýip umyt edýäris! �� Siziň pikiriňiz biziň we beýleki müşderilerimiz üçin möhümdir. Syn bermek we tejribäňizi paýlaşmak üçin biraz wagt aýyryp bilersiňizmi?",
      rateNotif2:"Jemgyýetimiziň gymmatly bölegi bolanyňyz üçin minnetdar!",
      goOrder:"Sargyt sahypasyna giriň",
      information:"Maglumatlar",
      orderLaw: `<ul class='list-disc ml-5'>
                  <li>Eltip bermek hyzmaty Aşgabat şäheriniň çägi bilen bir hatarda Büzmeýine we Änew şäherine hem elýeterlidir;</li>
                  <li>Sargydyň iň pes çägi 50 manat bolmaly; 50-150 manat arasy eltip bermek hyzmatynyň tölegi 10 manat; sargydyňyz 150 manatdan geçse eltip bermek hyzmaty mugt;</li>
                  <li>Saýtdan sargyt edeniňizden soňra operator size jaň edip sargydy tassyklar (eger hemişelik müşderi bolsaňyz sargytlaryňyz islegiňize görä awtomatik usulda hem tassyklanýar);</li>
                  <li>Sargydy barlap alanyňyzdan soňra töleg amala aşyrylýar. Eltip berijiniň size gowşurýan töleg resminamasynda siziň tölemeli puluňyz bellenendir. Töleg nagt görnüşde milli manatda amala aşyrylýar. Kabul edip tölegini geçiren harydyňyz yzyna alynmaýar.</li>
                </ul>`,
      aboutUsMain: 'Lybas web sahypamyza hoş geldiňiz! Bu ýerde zehinli ýerli atelýe we tikinçiler tarapyndan elde ýasalan ajaýyp köýnek we aksesuarlar görnüşinde türkmen gelin-gyzlarymyzyň ussatlygy janlanýar. Sizi däp-dessur bilen döwrebaplyk bir ara gelýän we her el işiň özboluşly bir hekaýasy bolan wirtual bazarymyza syýahat etmäge çagyrýarys.',
      aboutUsTitle1: 'Biziň görüşimiz:',
      aboutUsBody1: 'Lybasda türkmen zenan telekeçileriniň kämilligi dünýä jemgyýetçiligi bilen tanyşdyrylýar. Biziň platformamyz, Türkmenistanyň baý medeni mirasyny görkezmek bilen bir hatarda özboluşly el işleri döretmäge yhlas edýän ýerli atelýeleriň we tikinçileriň döredijiliginiň we ussatlygynyň subutnamasydyr.',
      aboutUsTitle2: 'Zenan telekeçileriň işini giňeltmek:',
      aboutUsBody2: 'Biziň bazarymyz diňe onlaýn söwda etmek üçin ýer bolman, eýsem türkmen zenanlarynyň telekeçilik ruhunyň aýnasydyr. El bilen ýasalan köýnekleriň we aksesuarlaryň dürli ýygyndysyny düzmek bilen, ýerli atelýe we tikinçilere öz ukybyny görkezmek we has giň auditoriýa ýüzlenmek üçin platforma hödürleýäris. Biz bilen söwdaňyz diňe bir geýimleriňize özboluşly bir element goşman, eýsem yhlasly aýal-gyzlarymyzyň telekeçilik işiniň ösmegine we giňelmegine hem kömek eder.',
      aboutUsTItle3: 'Taýsyz milli el işleri:',
      aboutUsBody3: 'Saýlanan köýneklerimiz we aksesuarlarymyz bilen türkmen milli modasynyň özüne çekijiligini öwreniň. Her bir iş, gadymy usullary häzirki zaman stilleri bilen birleşdirýän zenanlarymyzyň söýgisiniň miwesidir. Çylşyrymly nagyşlar bilen bezelen reňkli köýneklerden başlap, özboluşlygyňyzy görkezýän aksesuarlara çenli dükanymyz elde ýasalan täsinlikleriň hazynasydyr.',
      aboutUsTitle4: 'Ýakymly onlaýn söwda:',
      aboutUsBody4: 'Söwda syýahatyňyzda oňaýlylygyň möhümdigine düşünýäris. Biziň platformamyz aňsat gözlemäge, saýlamaga we söwda etmäge mümkinçilik berýär. Ulanyjy üçin amatly interfeýs bilen dürli görnüşdäki el işlere göz aýlap, döredijiligiň aňyrsyndaky zehinli zenanlar hakda okap, öýüňizden çykman söwda edip bilersiňiz.',
      aboutUsBody5: 'Lybasa agza bolanyňyz üçin sag boluň. Kolleksiýalara göz aýlanyňyzda, her bir ajaýyp el işleri görüp begenersiňiz we türkmen zenan telekeçileriniň gülläp ösýän jemgyýetine   goldaw bermäge buýsanarsyňyz diýip umyt edýäris. Däp-dessurlaryň, mümkinçilikleriň we stiliň birleşýän dünýäsine hoş geldiňiz - Lybasa hoş geldiňiz!',
      deliveryAndPaymentHead: "Eltip bermek we töleg tertibi",
      deliveryAndPaymentBody: `        
                              <li>Eltip bermek we töleg tertibi</li>
                              <div class="ml-5">Aşgabat: 20 manat</div>
                              <div class="ml-5">Änew: 30-40 manat</div>
                              <div class="ml-5">Büzmeýin: 50 manat</div>
                              <div class="ml-5">Çoganly: 30-40 manat</div>
                              <div class="ml-5">Gökdepe: 50 manat</div>
                              <div class="ml-5">Gurtly: 25 manat</div>
                              <div class="ml-5">Ýalkym: 40 manat</div>
                              <div class="ml-5">Tejen: 50 manat</div>
                              <li>Welaýatlara eltip bermek üçin 2-5 gün gerek bolup biler. Eltip bermek bahasy, ýaşaýan ýeriňize baglylykda 50-90 manat aralygynda bolup biler.</li>
                              <li>Web sahypamyzda harydy sarganyňyzdan soň, operatorymyz buýrugy tassyklamak üçin size jaň eder.</li>
                              <li>Web sahypamyzda görkezen salgy we telefon belgiňize görä eltip beriş amala aşyrylýar.</li>
                              <li>Sargyt tassyklanmasa, buýruk hasaba alynmaýar we ýerine ýetirilmeýär. Sargydyňyzy tassyklamak üçin beren telefon belgiňize gelýän jaňa jogap bermegiňizi haýyş edýäris. Şeýle hem, beren salgyňyzyň dogrudygyny barlaň.</li>
                              <li>Sargydy barlanyňyzdan we kabul edeniňizden soň töleg amala aşyrylýar. Kurýeriň töleg resminamasy tölegiň mukdaryny görkezýär. Eltip beriş hyzmadynyň bahasy önümiň umumy mukdaryna goşular. Töleg milli walýutada nagt görnüşinde amala aşyrylýar. Kabul edilen we tölenen haryt yzyna alynmaýar.</li>
                              <li>Sargyt tassyklanylandan soň 24 sagadyň dowamynda sargyt eýesini tapyp bilmesek, buýruk ýatyrylýar.</li>`
    }
  }
}

i18n
  .use(initReactI18next) // passes i18n down to react-i18next
  .init({
    resources,
    lng: localStorage.getItem('lang'), // language to use, more information here: https://www.i18next.com/overview/configuration-options#languages-namespaces-resources
    // you can use the i18n.changeLanguage function to change the language manually: https://www.i18next.com/overview/api#changelanguage
    // if you're using a language detector, do not define the lng option

    interpolation: {
      escapeValue: false // react already safes from xss
    }
  });

export default i18n;
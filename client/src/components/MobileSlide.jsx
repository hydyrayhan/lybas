...
function MobileSlide({ children, open, setOpen, title }) {
  return (
    <>
      <div onClick={() => setOpen(false)} className={"mobile-slide_background fixed md:hidden z-[11] left-0 top-0 w-full bg-black transition-opacity " + (open ? 'opacity-50 h-full' : 'opacity-0 h-0')}></div>
      <div className={"mobile-slide_foreground fixed md:hidden z-[12] left-0 bottom-0 rounded-t-lg w-full bg-white transition-all " + (open ? 'h-[90vh]' : 'h-0')}>

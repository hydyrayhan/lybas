...
const columns = [
  { id: 'size', label: 'Ölçeg', minWidth: 170 },
];

export default function Size() {
  const dataSizes = useSelector((state) => state?.fetchSizes?.data);
  const count = useSelector((state) => state?.fetchSizes?.count);
  const page = useSelector((state) => state?.fetchSizes?.page)
  const limit = useSelector((state) => state?.fetchSizes?.limit);
  const progress = useSelector((state) => state?.fetchSizes?.loading);
    ...
    if (type === 'add' && !data.id) {
      if (data.size) {
        if (!loadingDialog) {
          ...
          try {
            const { status } = await AxiosInstance.post('/sizes/add', data)
            if (status === 201) {
              dispatch(getSizes({ page: 0, limit }));
      ...
      setLoadingDialog(true);
      try {
        const {status} = await AxiosInstance.patch('/sizes/'+data.id,data);
        if(status === 200){
          dispatch(getSizes({ page, limit }));
          setOpenDialog(false);
          setLoadingDialog(false);
  ...
  const handleChangeRowsPerPage = (event) => {
    dispatch(getSizes({ page: 0, limit: event.target.value }))
  };
  ...
  const deleteRow = async (id) => {
    ...
    if (id === 'delete') {
      try {
        setLoadingDialog(true);
        const { status } = await AxiosInstance.post('/sizes/delete/' + deleteId)
        if (status === 200) {
          dispatch(getSizes({ page, limit }));
          setLoadingDialog(false);
          setDeleteDialog(false);
